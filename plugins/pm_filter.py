        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>درحال ارسال {message.text}. (ممکن است کمی زمانبر باشد) 🔍</i></b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b><i>درحال ارسال {message.text}. (ممکن است کمی زمانبر باشد) 🔍</i></b>", reply_to_message_id=message.id)
...
async def next_page(bot, query):
   ...
   # if not search:
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton('کیفیت ها', callback_data=f"qualities#{key}"),
                InlineKeyboardButton("قسمت ها", callback_data=f"episodes#{key}"),
                InlineKeyboardButton("فصل ها",  callback_data=f"seasons#{key}")
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton("ارسال همه", callback_data=f"sendfiles#{key}"),
            InlineKeyboardButton("سال انتشار", callback_data=f"years#{key}")
        ])
        btn.insert(0, [
            InlineKeyboardButton("زیرنویس و دوبله", callback_data=f"languages#{key}")
        ])
    else:
        btn = []
        btn.insert(0, 
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("صفحه", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("صفحه بعد", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
                        InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"),
                        InlineKeyboardButton("صفحه بعد", callback_data=f"next_{req}_{key}_{n_offset}")
                    ],
                )
        else:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("صفحه", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("صفحه بعد", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>درحال ارسال {message.text}. (ممکن است کمی زمانبر باشد) 🔍</i></b>")
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="سال انتشار را وارد کنید", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="بازگشت به صفحه اصلی", callback_data=f"fy#homepage#{key}")])
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("صفحه", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="صفحه بعد",callback_data=f"next_{req}_{key}_{offset}")]
                )
    
            else:
                btn.append(
                    [InlineKeyboardButton("صفحه", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="صفحه بعد",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
...
async def episodes_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="قسمت موردنظر را انتخاب کنید", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="بازگشت به صفحه اصلی", callback_data=f"fe#homepage#{key}")])
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="👇 از طریق دکمه های زیر انتخاب کنید ", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="بازگشت به صفحه اصلی", callback_data=f"fl#homepage#{key}")])
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="فصل موردنظر را انتخاب کنید", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="بازگشت به صفحه اصلی", callback_data=f"next_{req}_{key}_{offset}")])
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="کیفیت موردنظر را انتخاب کنید", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="صقحه دیگری وجود ندارد",callback_data="pages")]
        )
    if lang != "homepage":
        req = query.from_user.id
        offset = 0
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        ...
        btn.append([InlineKeyboardButton(text="❌ بستن", callback_data=f'spol#{reqstr1}#close_spellcheck')])
