...
class script(object):
    START_TXT = """<b>فیلمیا، هوش مصنوعی فیلم و سریال
    
    کافیه اسم فیلم، سریال مورد نظرتو به انگلیسی برام بفرستی... 😎</b>"""
...
🌟 امتیاز: <a href={url}/ratings>{rating}</a> / 10 (بر اساس {votes} رای در IMDB.)
☀️ زبان : <code>{languages}</code>
📀 مدت زمان: {runtime} دقیقه
🎛 کشور ها : <code>{countries}</code>
